...
Planning more than one character? Prompt overrides resolve through install-global layers — ship each character's prompt set as its own small external plugin and toggle it per MO2 profile ([multiple playthroughs](docs/multi-playthroughs.md)).
...
| [`output entries/`](<output entries/_manual.md>) | the frozen token-template pack — historical source, per-step detail |
| [`output package/`](<output package/README.md>) | the current build target — the importer-ready package reworked from the frozen trees |
