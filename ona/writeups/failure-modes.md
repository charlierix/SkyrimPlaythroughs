...
## 7. the sixth-member latch (seen in play)
The registered virtual entity Ohne drifted into roleplaying a physical follower — a chatty voice with an imagined body, location, and follower opinions — and the folds began treating it as a party member they could address. Causes stack: an enabled virtual is a listed conversation participant (scene context lists it, the gamemaster may pick it as a speaker, the chat audience can target it), and nothing in its context said otherwise.
Rule: anything the prompt tree can address needs an explicit self-model, or it defaults to follower-shaped. Standing rules for non-body entities are always_inject knowledge, not memories — a folds-side fence (no separate Ohne to talk to; the mind speaks only through bodies) plus the entity's own rule (no body, never speaks as a separate voice). Memories from the misbehaving stretch get deleted, not argued with — stored follower-roleplay keeps teaching the role. The deterministic fallback is disabling the entity between uses: bonds render from stored link data, not the live entity (verify once with a read-back).